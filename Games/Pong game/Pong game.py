...
while True:
	...
	if ball.xcor() > 390:
		...
		if player_1_score > 10:
			...
			player_1_score = 0
			player_2_score = 0
			pen.write(f'Player 1 Score : {player_1_score}   Player 2 Score : {player_2_score}',align='center',font=('courier',18,'bold'))
			ball.dx *= -1
		else:
			pen.write(f'Player 1 Score : {player_1_score}   Player 2 Score : {player_2_score}',align='center',font=('courier',18,'bold'))
			ball.dx *= -1
	...
	if (ball.xcor() > 340 and ball.xcor() < 350) and ball.ycor() in range(paddle_b.ycor()-50, paddle_b.ycor()+50):
		ball.setx(340)
		ball.dx *= -1
		winsound.PlaySound('bounce.wav', winsound.SND_ASYNC)


	if (ball.xcor() < -340 and ball.xcor() > -350 ) and ball.ycor() in range(paddle_a.ycor()-50, paddle_a.ycor()+50):
		ball.setx(-340)
		ball.dx *= -1
		winsound.PlaySound('bounce.wav', winsound.SND_ASYNC)
